...
static struct keys_chardev* keys_devp = NULL;
#endif
...
#ifdef ADD_KEYS_CHRDEV
static int adc_keys_chardev(void)
{
    int error;
    dev_t devno;
    ...
    if (error) {
        printk("ERROR: alloc_chrdev_regin failed.\n");
        return error;
    }

    keys_devp = kmalloc(sizeof(struct keys_chardev), GFP_KERNEL);
    if (NULL == keys_devp) {
        printk("ERROR: kmallloc failed.\n");
        error = -ENOMEM;
        goto error_unregister_chrdev_region;
    }
    memset(keys_devp, 0, sizeof(struct keys_chardev));

    cdev_init(&keys_devp->cdev, &keys_fops);
    keys_devp->cdev.owner = THIS_MODULE;
    error = cdev_add(&keys_devp->cdev, devno, 1u);
    if (error) {
        printk("ERROR: cdev_add failed.\n");
        goto error_kfree;
    }

    memcpy(&keys_devp->devno, &devno, sizeof(dev_t));

    keys_devp->class = class_create(THIS_MODULE, DEVICE_NAME);
    if(IS_ERR(keys_devp->class)) {
        printk("ERROR: class_create failed.\n");
        error = -EFAULT;
        goto error_cdev_del;
    }
    device_create(keys_devp->class, NULL, devno, NULL, DEVICE_NAME);
    printk("register chardev %s success.\n", DEVICE_NAME);
